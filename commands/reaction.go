...
func Reaction(slackToken string, targetChannels []string, targetEmoji []string, eachReport bool, allReportChannel string, oldest, latest DateTime) error {
	channelFilter := NewStringFilter(targetChannels)
	emojiFilter := NewStringFilter(targetEmoji)
